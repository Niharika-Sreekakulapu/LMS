...
public class AuthServiceImpl implements AuthService {
    ...
    public User approveUser(Long id) {
        ...
        String body = "<!DOCTYPE html>"
                ...
                + "<title>Account Approved - LMS</title>"
                + "</head>"
                + "<body style='margin: 0; padding: 0; font-family: Inter, Arial, sans-serif; background-color: #F5F2EB; color: #3E2723;'>"
                ...
                + "<!-- Header -->"
                + "<div style='background: linear-gradient(135deg, #D7CCC8 0%, #F5F2EB 100%); padding: 40px 30px; text-align: center;'>"
                + "<h1 style='margin: 0; font-size: 28px; font-weight: 800; color: #3E2723;'>"
                + "<span style='font-size: 32px;'>📚</span> LMS Library</h1>"
                + "<p style='margin: 10px 0 0 0; font-size: 16px; color: #5D4037;'>Welcome to Your Digital Library</p>"
                + "</div>"
                + "<!-- Content -->"
                + "<div style='padding: 40px 30px;'>"
                + "<h2 style='margin: 0 0 20px 0; font-size: 24px; font-weight: 700; color: #3E2723;'>🎉 Account Approved!</h2>"
                + "<p style='margin: 0 0 20px 0; font-size: 16px; line-height: 1.6; color: #3E2723;'>Hi " + saved.getName() + ",</p>"
                + "<p style='margin: 0 0 20px 0; font-size: 16px; line-height: 1.6; color: #3E2723;'>"
                + "Congratulations! Your registration request has been reviewed and <strong style='color: #2E7D32;'>approved</strong> by our administrators.</p>"
                + "<div style='background: linear-gradient(135deg, #E8F5E8 0%, #F1F8E9 100%); border-left: 4px solid #2E7D32; padding: 20px; margin: 30px 0; border-radius: 8px;'>"
                + "<h3 style='margin: 0 0 10px 0; font-size: 18px; color: #2E7D32;'>Next Steps:</h3>"
                + "<ul style='margin: 0; padding-left: 20px; color: #3E2723;'>"
                + "<li style='margin-bottom: 8px;'>Your account is now active and ready to use</li>"
                + "<li style='margin-bottom: 8px;'>Log in with your registered email and password</li>"
                + "<li style='margin-bottom: 8px;'>Complete your profile to enhance your library experience</li>"
                + "<li>Explore our collection of books and start borrowing!</li>"
                + "</ul>"
                + "</div>"
                + "<p style='margin: 30px 0 0 0; font-size: 16px; line-height: 1.6; color: #3E2723;'>"
                + "If you have any questions, feel free to contact our support team.</p>"
                + "</div>"
                + "<!-- Footer -->"
                + "<div style='background-color: #F5F2EB; padding: 30px; text-align: center; border-top: 1px solid #D7CCC8;'>"
                + "<p style='margin: 0; font-size: 14px; color: #5D4037;'>"
                + "Thank you for choosing <strong>LMS Library</strong><br>"
                + "Happy Reading! 📖</p>"
                + "</div>"
                + "</div>"
                + "</body>"
    ...
    public User rejectUser(Long id, String reason) {
        ...
        String body = "<!DOCTYPE html>"
                ...
                + "<meta charset='UTF-8'>"
                + "<meta name='viewport' content='width=device-width, initial-scale=1.0'>"
                + "<title>Registration Update - LMS</title>"
                + "</head>"
                + "<body style='margin: 0; padding: 0; font-family: Inter, Arial, sans-serif; background-color: #F5F2EB; color: #3E2723;'>"
                + "<div style='max-width: 600px; margin: 0 auto; background-color: #FFFFFF; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.1);'>"
                + "<!-- Header -->"
                ...
                + "<h1 style='margin: 0; font-size: 28px; font-weight: 800; color: #3E2723;'>"
                + "<span style='font-size: 32px;'>📚</span> LMS Library</h1>"
                + "<p style='margin: 10px 0 0 0; font-size: 16px; color: #5D4037;'>Library Management System</p>"
                + "</div>"
                + "<!-- Content -->"
                + "<div style='padding: 40px 30px;'>"
                + "<h2 style='margin: 0 0 20px 0; font-size: 24px; font-weight: 700; color: #3E2723;'>Registration Update</h2>"
                + "<p style='margin: 0 0 20px 0; font-size: 16px; line-height: 1.6; color: #3E2723;'>Hi " + saved.getName() + ",</p>"
                + "<p style='margin: 0 0 20px 0; font-size: 16px; line-height: 1.6; color: #3E2723;'>"
                + "Thank you for your interest in joining the LMS Library. After careful review, we regret to inform you that your registration request has been <strong style='color: #D32F2F;'>declined</strong>.</p>"
                + (reason != null && !reason.trim().isEmpty() ?
                    "<div style='background: linear-gradient(135deg, #FFEBEE 0%, #FFCDD2 100%); border-left: 4px solid #D32F2F; padding: 20px; margin: 30px 0; border-radius: 8px;'>"
                    + "<h3 style='margin: 0 0 10px 0; font-size: 18px; color: #D32F2F;'>Reason for Decision:</h3>"
                    + "<p style='margin: 0; font-size: 16px; line-height: 1.6; color: #3E2723;'>" + escapeHtml(reason) + "</p>"
                    + "</div>" : "")
                + "<div style='background: linear-gradient(135deg, #FFF3E0 0%, #FFE0B2 100%); border-left: 4px solid #F57C00; padding: 20px; margin: 30px 0; border-radius: 8px;'>"
                + "<h3 style='margin: 0 0 10px 0; font-size: 18px; color: #F57C00;'>What You Can Do:</h3>"
                + "<ul style='margin: 0; padding-left: 20px; color: #3E2723;'>"
                + "<li style='margin-bottom: 8px;'>Review the requirements for library membership</li>"
                + "<li style='margin-bottom: 8px;'>Ensure all submitted documents meet our standards</li>"
                ...
                + "</div>"
                + "<p style='margin: 30px 0 0 0; font-size: 16px; line-height: 1.6; color: #3E2723;'>"
                + "We appreciate your interest in our library services. If you have any questions or need assistance, please don't hesitate to reach out to our support team.</p>"
                + "</div>"
                + "<!-- Footer -->"
                + "<div style='background-color: #F5F2EB; padding: 30px; text-align: center; border-top: 1px solid #D7CCC8;'>"
                + "<p style='margin: 0; font-size: 14px; color: #5D4037;'>"
                + "Thank you for considering <strong>LMS Library</strong><br>"
                + "We hope to serve you in the future! 📚</p>"
                + "</div>"
                + "</div>"
                + "</body>"
                + "</html>";
